...
const shukujitu = (y, m, mon) => {
  ...
  // 指定月の祝日を抽出
  let monHoliday = holidayAry[m];
  // 振替休日の設定

  if (monHoliday.length) {
    // 月曜から日曜日の余り数字をだす
    const sch = (mon + 6) % 7;
    // 振替休日のチェック
    holidayAry[m].forEach((n) => {
      if (n % 7 == sch) {
        while (monHoliday.includes(n)) n++;
        monHoliday.push(n);
      }
    });
  }
...
const createCalendar = (year, month, today) => {
  ...
  if (year < 2025) {
    ...
    if (month == 0) {
      ...
      readText += "、1日から";
      // 1月6日が下曜なら5日日曜は営業するので調整
      if (mon1st != 6) {
        calArray[5].push("closed");
        readText += "5日";
      } else {
        readText += "4日";
      }
    ...
    for (let d = mon1st; d <= month_end; d += 7) {
      ...
      if (!(month == 11 && d >= 25)) {
        // 火曜1日休業
        if (d == 7 && calArray[1].length == 0) {
          calArray[1].push("closed");
          readText += "、1日";
        }
        // 月曜休業
        if (!calArray[d].includes("closed")) {
          calArray[d].push("closed");
          readText += "、" + d + "日";
        }
        // 火曜休業
        ...
        // 8月夏休み
        if (month == 7 && d >= 15 && d <= 21) {
          calArray[d + 2].push("closed");
          readText += "、" + (d + 2) + "日";
        }
      }
    }
